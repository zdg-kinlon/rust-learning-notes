...
并发就是**同时处理多个任务**，但这里的“同时”是相对的。在单核心 CPU 上，多个任务实际上是交替执行的，只是速度很快，所以看起来像同时进行的。

#### 并行（Parallelism）

并行是**真正的多个任务同时运行**，这通常发生在多核心或多处理器系统中。

每个任务都在自己的CPU核心上独立执行，没有相互等待。
...
> 
> - 并发和并行都是为了提高程序的执行效率
> - 进程是程序运行的实例，线程是进程中的执行单元
> - 协程和绿色线程是轻量级的线程，用于提高并发性能
> - 1:1 模型是 Rust 中线程和操作系统线程的对应关系，而 M:N 模型则是一种更高效的并发模型。
